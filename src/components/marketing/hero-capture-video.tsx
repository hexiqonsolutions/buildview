...
export function HeroCaptureVideo({ className }: { className?: string }) {
  return (
    <div className={cn("absolute inset-0 z-0 overflow-hidden bg-brand-primary", className)} aria-hidden>
      {/* Atmospheric base — visible if video is missing */}
      <div className="absolute inset-0 mesh-gradient" />
      <div className="dot-pattern absolute inset-0 opacity-30" />

      <video
        className="absolute inset-0 h-full w-full scale-105 object-cover motion-safe:animate-hero-ken"
        autoPlay
        muted
        loop
        playsInline
